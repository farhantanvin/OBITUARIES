...
class AppNavbar extends React.Component {
	...
	componenDidUpdate(prevProps) {
		...
		if (this.props !== prevProps) {
			this.setState({ showNavbar: !(window.location.pathname.includes('sign-up') || window.location.pathname.includes('sign-in')) });
		}
	}
	...
	render() {
		const getLink = (path, text) => (
			<LinkContainer
				onClick={() => { this.setMobileNav(false); }}
				to={path}
		...
		return (
			<Navbar collapseOnSelect expand="md" variant="dark" className={Styles.navbar}>
				<Link to="/dashboard" onClick={() => { this.setMobileNav(false); }} className={Styles.navLogo}>
					<img src={Logo} alt="May Cherished  Memories Bring You Moments of Comfort" />
				</Link>
				<Navbar.Toggle
					className={Styles.navbarToggle}
					onClick={() => { this.setMobileNav(true); }}
				...
				<Navbar.Collapse id="navbar-collapse" className={`navbar-collapse ${Styles.navbarCollapse} ${(this.state.mobileNavBar) ? (Styles.showNavbar) : (Styles.collapseNavbar)}`}>
					<Navbar.Toggle
